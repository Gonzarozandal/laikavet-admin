  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (propietarioEditando) {
        const response = await fetch(`http://localhost:3000/api/duenos/${propietarioEditando._id || propietarioEditando.id}`, {
          method: 'PUT',
          headers: {
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm mt-2">
                {searchTerm ? 'Intenta con otro término de búsqueda' : 'Haz clic en "Agregar Propietario" para comenzar'}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white text-lg font-bold flex-shrink-0">
                          ...
                          {(propietario.apellidoDueno?.[0] || 'U').toUpperCase()}
                        </div>

                        {/* Nombre y DNI */}
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg font-semibold text-gray-900 dark:text-white truncate">
                            {propietario.nombreDueno} {propietario.apellidoDueno}
                          </h3>
                          {propietario.dni && (
